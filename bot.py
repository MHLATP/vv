...
async def discord_log_callback(msg: str, level: str):
    """Chỉ in log ra màn hình console thay vì gửi vào Discord Channel."""
    # Filter out noisy levels (debug, progress, info) to avoid flooding
    ...
    # In trực tiếp ra terminal
    print(f"{emoji} [{level.upper()}] {msg}")


# ── Discord Summary Report Callback ───────────────────────────────────────────
async def discord_report_callback(user_id: str, username: str, avatar_url: str, results: list):
    """Chỉ in báo cáo tổng kết ra màn hình console."""
    success_count = sum(1 for r in results if r["success"])
    total_count = len(results)
    
    print(f"🎁 Quest Auto-Complete hoàn tất cho tài khoản @{username}!")
    print(f"📜 Kết quả: {success_count}/{total_count} thành công.")
